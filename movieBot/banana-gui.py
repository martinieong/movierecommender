...
class TravelBotGui:
    def __init__(self):
        ...
        self.base_prompt_files = ["./movie_rec.txt", "./movie_trivia.txt", "./movie_gen.txt"]
        ...
        self.movie_gen_file = self.base_prompt_files[2]
        self.entry_separator = "###"

        self.prompt_options_labels = ["Give movie recommendations", "Answer movie trivia", "Talk about movie plots"]
        self.bad_words = []
    ...
    def window(self):
        ...
        with base_col1:
            ...
            self.text_input = self.st.text_input("Ask a question here", value="I like The Matrix and John Wick, what is another movie I would like?")
            self.st.slider("Tokens to generate", min_value=10, max_value=100, value=40, key="max_length")

        with self.st.expander("Advanced Options"):
            self.st.slider("Age", min_value=10, max_value=80, value=18, key="age")
            self.st.selectbox("Gender",  ('male', 'female'), key='gender')
            self.st.slider("Temperature", min_value=0.0, max_value=2.0, value=0.7, key="temperature")
        ...
        with col1:
            has_generated = self.st.button("Generate Response", on_click=self.generate_all_new, disabled=disable_buttons)
        with col2:
    ...
    def generate_all_new(self):
        ...
        with self.st.spinner("Generating..."):
            final_input = f"You are talking to a {age} year old {gender}\n" + self.base_prompt + self.text_input + extra_stuff
    ...
    def generate_text(self, data):
        api_key = 'abcdef' # Not the actual API key, masked for privacy
        model_key = "gptj"
    ...
    def execute(self, utterance, end_sequence="###", bad_words=[]):
        ...
        if "age" not in self.st.session_state:
            self.st.session_state["age"] = 35
        age = self.st.session_state["age"]
        if "gender" not in self.st.session_state:
            self.st.session_state["gender"] = 'male'
        gender = self.st.session_state["gender"]
        if "max_length" not in self.st.session_state:
            self.st.session_state["max_length"] = 40
        max_length = self.st.session_state["max_length"]
        if "temperature" not in self.st.session_state:
            self.st.session_state["temperature"] = 1.0
        ...
        if "top_p" not in self.st.session_state:
            self.st.session_state["top_p"] = 0.65
        ...
        data = {"text": f"{utterance}", "length": max_length, "temperature": temperature,
                "topK": top_k, "topP": top_p}
        out = banana.run(api_key, model_key, data)

        prompt = out['modelOutputs'][0]['output'].split('###')[0]
        return prompt
